...
Remember, the CLT tells us (more or less) that if we take the mean from a sample and do this over and over again, 
the resulting sampling distribution (which is the distribution of the means from the sample) should begin to look like a normal curve. 
*/

/*
suppose to consider a population of 100,000 students from the faculties of modern literature in a country A. 
This population is special because 80 percent population are female (y = 1),
while only 20 percent are male (y = 0). Gender is zero-one variable that is 
...
almost see a bell, even though an asymmetric one. The central limit theorem is kicking
in now!
